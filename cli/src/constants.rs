// L1 clients
pub const GETH: &str = "geth (go)";
pub const ERIGON: &str = "erigon (go)";

// L2 clients
pub const OP_GETH: &str = "op-geth (go)";
pub const OP_ERIGON: &str = "op-erigon (go)";

// Rollup clients
pub const OP_NODE: &str = "op-node (rust)";
pub const MAGI: &str = "magi (rust)";

// Challenger agents
pub const OP_CHALLENGER_GO: &str = "op-challenger-go (go)";
pub const OP_CHALLENGER_RUST: &str = "op-challenger-rust (rust)";

// -------------------------------------------------------------------------------

// Git URLs
pub const OP_MONOREPO_URL: &str = "git@github.com:ethereum-optimism/optimism.git";
pub const OP_RS_MONOREPO_URL: &str = "git@github.com:refcell/optimism-rs.git";
pub const MAGI_REPO_URL: &str = "git@github.com:a16z/magi.git";
pub const OP_CHALLENGER_RUST_REPO_URL: &str = "git@github.com/clabby/op-challenger.git";
